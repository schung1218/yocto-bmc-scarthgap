...
IMAGE_CMD:spinand() {
	...
	if [ -f ${IMGDEPLOYDIR}/${IMAGE_NAME}.rootfs.ubi ]; then
		if [ "${SECURE_BOOT}" = "no" ]; then
			...
			rm -rf $(date "+%m%d-*");)
			if [ -f ${DEPLOY_DIR_IMAGE}/enc_bl2-ma35d0-spinand.dtb ]; then
				rm ${DEPLOY_DIR_IMAGE}/enc_bl2-ma35d0-spinand.dtb
				rm ${DEPLOY_DIR_IMAGE}/enc_bl2-ma35d0-spinand.bin
			fi
		else
			(cd ${DEPLOY_DIR_IMAGE}; \
			...
			cp conv/enc_bl2-ma35d0.dtb enc_bl2-ma35d0-spinand.dtb; \
			cp conv/enc_bl2-ma35d0.bin enc_bl2-ma35d0-spinand.bin; \
			...
			jq 'setpath(["image",1,"file"];"enc_bl2-ma35d0-spinand.dtb")' | \
			jq 'setpath(["image",2,"file"];"enc_bl2-ma35d0-spinand.bin")' > nuwriter/pack-spinand.json); \
			nuwriter/nuwriter -p nuwriter/pack-spinand.json; \
...
IMAGE_CMD:spinor() {
	...
	if [ -f ${IMGDEPLOYDIR}/${IMAGE_NAME}.rootfs.jffs2 ]; then
		if [ "${SECURE_BOOT}" = "no" ]; then
			...
			rm -rf $(date "+%m%d-*");)
			if [ -f ${DEPLOY_DIR_IMAGE}/enc_bl2-ma35d0-spinor.dtb ]; then
				rm ${DEPLOY_DIR_IMAGE}/enc_bl2-ma35d0-spinor.dtb
				rm ${DEPLOY_DIR_IMAGE}/enc_bl2-ma35d0-spinor.bin
			fi
		else
			(cd ${DEPLOY_DIR_IMAGE}; \
			ln -sf ${IMGDEPLOYDIR}/${IMAGE_NAME}.rootfs.ext4 rootfs.ext4-sdcard; \
			...
			cp conv/enc_bl2-ma35d0.dtb enc_bl2-ma35d0-spinor.dtb; \
			cp conv/enc_bl2-ma35d0.bin enc_bl2-ma35d0-spinor.bin; \
			echo "{\""publicx"\": \""$(head -6 conv/header_key.txt | tail +6)"\", \
			\""publicy"\": \""$(head -7 conv/header_key.txt | tail +7)"\", \
			\""aeskey"\": \""$(head -2 conv/header_key.txt | tail +2)"\"}" | \
			jq  > nuwriter/otp_key-spinor.json; \
			$(cat ${NUWRITER_DIR}/pack-spinor.json | \
			jq 'setpath(["image",1,"file"];"enc_bl2-ma35d0-spinor.dtb")' | \
			jq 'setpath(["image",2,"file"];"enc_bl2-ma35d0-spinor.bin")' > nuwriter/pack-spinor.json); \
			nuwriter/nuwriter -p nuwriter/pack-spinor.json; \
   ...
   (cd ${DEPLOY_DIR_IMAGE}; ubinize ${UBINIZE_ARGS} -o u-boot-initial-env.ubi-nand u-boot-initial-env-nand-ubi.cfg)
	...
	if [ -f ${IMGDEPLOYDIR}/${IMAGE_NAME}.rootfs.ubi ]; then
		if [ "${SECURE_BOOT}" = "no" ]; then
			...
			rm -rf $(date "+%m%d-*");)
			if [ -f ${DEPLOY_DIR_IMAGE}/enc_bl2-ma35d0-nand.dtb ]; then
				rm ${DEPLOY_DIR_IMAGE}/enc_bl2-ma35d0-nand.dtb
				rm ${DEPLOY_DIR_IMAGE}/enc_bl2-ma35d0-nand.bin
			fi
		else
			(cd ${DEPLOY_DIR_IMAGE}; \
			ln -sf ${IMGDEPLOYDIR}/${IMAGE_NAME}.rootfs.ubi rootfs.ubi-nand; \
			...
			cp conv/enc_bl2-ma35d0.dtb enc_bl2-ma35d0-nand.dtb; \
			cp conv/enc_bl2-ma35d0.bin enc_bl2-ma35d0-nand.bin; \
			echo "{\""publicx"\": \""$(head -6 conv/header_key.txt | tail +6)"\", \
			\""publicy"\": \""$(head -7 conv/header_key.txt | tail +7)"\", \
			\""aeskey"\": \""$(head -2 conv/header_key.txt | tail +2)"\"}" | \
			jq  > nuwriter/otp_key-nand.json; \
			$(cat ${NUWRITER_DIR}/pack-nand.json | \
			jq 'setpath(["image",1,"file"];"enc_bl2-ma35d0-nand.dtb")' | \
			jq 'setpath(["image",2,"file"];"enc_bl2-ma35d0-nand.bin")' > nuwriter/pack-nand.json); \
			nuwriter/nuwriter -p nuwriter/pack-nand.json; \
...
IMAGE_CMD:sdcard() {
	...
	if [ -f ${IMGDEPLOYDIR}/${IMAGE_NAME}.rootfs.ext4 ]; then
		...
		if [ "${SECURE_BOOT}" = "no" ]; then
			...
			rm -rf $(date "+%m%d-*");)
			if [ -f ${DEPLOY_DIR_IMAGE}/enc_bl2-ma35d0-sdcard.dtb ]; then
				rm ${DEPLOY_DIR_IMAGE}/enc_bl2-ma35d0-sdcard.dtb
				rm ${DEPLOY_DIR_IMAGE}/enc_bl2-ma35d0-sdcard.bin
			fi
		else
			(cd ${DEPLOY_DIR_IMAGE}; \
			ln -sf ${IMGDEPLOYDIR}/${IMAGE_NAME}.rootfs.ext4 rootfs.ext4-sdcard; \
			...
			nuwriter/nuwriter -c nuwriter/header-sdcard.json; \
			cp conv/enc_bl2-ma35d0.dtb enc_bl2-ma35d0-sdcard.dtb; \
			cp conv/enc_bl2-ma35d0.bin enc_bl2-ma35d0-sdcard.bin; \
			echo "{\""publicx"\": \""$(head -6 conv/header_key.txt | tail +6)"\", \
			\""publicy"\": \""$(head -7 conv/header_key.txt | tail +7)"\", \
			\""aeskey"\": \""$(head -2 conv/header_key.txt | tail +2)"\"}" | \
			...
			jq 'setpath(["image",2,"file"];"enc_bl2-ma35d0-sdcard.dtb")' | \
			jq 'setpath(["image",3,"file"];"enc_bl2-ma35d0-sdcard.bin")' | \
		...
		if [ "${SECURE_BOOT}" = "no" ]; then
			...
			# 0x20000
			dd if=${DEPLOY_DIR_IMAGE}/bl2-ma35d0.dtb of=${SDCARD} conv=notrunc seek=256 bs=512
			# 0x30000
			dd if=${DEPLOY_DIR_IMAGE}/bl2-ma35d0.bin of=${SDCARD} conv=notrunc seek=384 bs=512
		else
			# 0x400
			dd if=${DEPLOY_DIR_IMAGE}/header-${IMAGE_BASENAME}-${MACHINE}-enc-sdcard.bin of=${SDCARD} conv=notrunc seek=2 bs=512
			# 0x20000
			dd if=${DEPLOY_DIR_IMAGE}/enc_bl2-ma35d0-sdcard.dtb of=${SDCARD} conv=notrunc seek=256 bs=512
			# 0x30000
			dd if=${DEPLOY_DIR_IMAGE}/enc_bl2-ma35d0-sdcard.bin of=${SDCARD} conv=notrunc seek=384 bs=512
		fi
		# 0x40000
