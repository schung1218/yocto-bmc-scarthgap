...
DESCRIPTION = "ma35d0 U-Boot suppporting ma35d0 ev boards."
#SECTION = "bootloaders"
require recipes-bsp/u-boot/u-boot.inc

PROVIDES += "u-boot"
DEPENDS += "dtc-native bc-native flex-native bison-native u-boot-scr-ma35d0"
...
do_compile:append() {
        ...
        for config in ${UBOOT_MACHINE}; do
			...
			if echo ${TFA_DTB} | grep -q "512"; then
                        ...
                        if [ "${type}" = "sdcard" ]; then
                            if [ "${config}" = "ma35d0_sdcard0_defconfig" ]; then
